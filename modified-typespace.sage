...
from sage.modular.local_comp.liftings import lift_gen_to_gamma1, lift_ramified
...
def find_in_space(f, A, base_extend=False):
    ...
    M = f.elliptic_curve().modular_symbol_space()

    if base_extend:
        K = M.eigenvalue(1).parent()
        D = A.base_extend(K)
    else:
...
class TypeSpaceModified(SageObject):
    r"""
    ...
    def __init__(self, f, p, base_extend=True):
        ...
        bvecs,bvecs_small = [],[]
        w = [a for a in range(1,p**self.u()) if gcd(a,p) == 1]
        matpowers = [mat**a for a in w] # only considering the primitive characters. i.e., characters
        # of conductor p^u. Note the difference from the original code, which considres all chars of conductor dividing p^u.
        for v in self.e_space.free_module().basis():
            bvecs += mat.maxspin(v)

            for mat0 in matpowers:
                bvecs_small.append(v*mat0)
        T = V.submodule(bvecs)
        T_small = V.submodule(bvecs_small)
        ...
        self.t_space_small = amb.base_extend(R).submodule(T_small, check=False)
    ...
    def maximal_twist_level(self):
        NN = self.form().level()
        V = self.t_space_small
        A = V.ambient()

        while V.intersection(A.new_submodule()).dimension() == 0:
            NN = NN / self.prime()
